...
class AudioService implements IAudioService {
  ...
  AudioService({FlutterSoundRecorder? recorder}) : _recorder = recorder;

  FlutterSoundRecorder get _nativeRecorder =>
  ...
  Future<void> init() async {
    ...
    try {
      ...
      await _nativeRecorder.openRecorder();

      // ── stream سطح صدا از flutter_sound ──
      _nativeRecorder.onProgress!.listen((event) {
      ...
      await _nativeRecorder.setSubscriptionDuration(
  ...
  Future<void> startRecording({RecordingConfig? config}) async {
    ...
    // Lazy initialization: native recorder setup must happen only when the
    // recording feature is actually used. This makes app startup resilient on
    // Android devices where audio codecs/plugins are problematic.
    if (!_isInitialized) {
      try {
        await init();
      } catch (e) {
        throw AudioServiceException(
          'امکان آماده‌سازی ضبط صدا وجود ندارد.',
    ...
    try {
      ...
      await _nativeRecorder.startRecorder(
        toFile: _filePath,
        codec: _config.codec,
  ...
  Future<RecordingInfo?> stopRecording() async {
    ...
    try {
      final recordedPath = await _nativeRecorder.stopRecorder();
  ...
  Future<void> pauseRecording() async {
    ...
    try {
      await _nativeRecorder.pauseRecorder();
  ...
  Future<void> cancelRecording() async {
    ...
    try {
      if (_nativeRecorder.isRecording || _nativeRecorder.isPaused) {
        await _nativeRecorder.stopRecorder();
      }
    } catch (_) {}
  ...
  Future<void> dispose() async {
    ...
    try {
      ...
      await _nativeRecorder.closeRecorder();
    } catch (e) {
